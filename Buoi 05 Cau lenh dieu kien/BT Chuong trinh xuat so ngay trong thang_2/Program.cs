...
namespace BT_Chuong_trinh_xuat_so_ngay_trong_thang_2
{
    class Program
    {
        ...
        nhap_so:
            Console.WriteLine("Nhập tháng và năm");
            if (int.TryParse(Console.ReadLine(), out thang) && int.TryParse(Console.ReadLine(), out nam) && thang > 0 && thang < 13 && nam > 0)
            {
                Console.WriteLine("tháng " + thang + " năm " + nam);
                if (nam % 4 == 0 && nam % 100 != 0)
                {
                    if (thang == 2) Console.WriteLine("Số ngày trong tháng là 29 ngày");
                    Console.ReadKey();
                    return;
                }
                switch (thang)
